...
class RegistrationModel extends FormModel
{
	...
	public function save($data): bool
	{
		...
		// Prüfen, ob bereits eine Registrierung für diesen User/Event existiert
		if ($this->isRegistered($data['event_id'], $user->id))
		{
			Log::add(
				'Duplicate registration attempt detected for user ' . $user->id . ' and event ' . $data['event_id'],
				Log::WARNING,
				'com_marathonmanager.registration'
			);
			$app->enqueueMessage(Text::_('COM_MARATHONMANAGER_ERROR_ALREADY_REGISTERED'), 'warning');
	...
	private function sendConfirmationMail($data): void
	{
		...
		if (!$params->get('registration_confirmation_mail_id', 0)){
			Log::add('Not send confirmation mail, registration_confirmation_mail_id is not set', Log::INFO, 'com_marathonmanager.registration');
			return;
		}

		// Sicherstellen, dass der Benutzer in AcyMailing existiert, bevor die Mail gesendet wird
		$newsletterModel = new NewsletterModel();
		$user = $newsletterModel->saveUser($data['contact_email'], $data['contact_first_name'], $data['contact_last_name'], $data['created_by'] ?? null);

		if (!$user) {
			Log::add('Could not save/get user in AcyMailing before sending confirmation mail: ' . $data['contact_email'], Log::WARNING, 'com_marathonmanager.registration');
			return;
		}

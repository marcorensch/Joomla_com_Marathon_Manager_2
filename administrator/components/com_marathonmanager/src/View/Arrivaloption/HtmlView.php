...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar(): void
	{
		...
		$isNew = (!$this->item->id);
        $toolbarButtons = [];
        ToolbarHelper::title($isNew ? Text::_('COM_MARATHONMANAGER_ARRIVAL_OPTION_NEW') : Text::_('COM_MARATHONMANAGER_ARRIVAL_OPTION_EDIT'), 'fas fa-car');
        ...
        if ($isNew) {
            ...
            {
                ToolbarHelper::apply('arrivaloption.apply');
                $toolbarButtons[] = ['save', 'arrivaloption.save'];
                // We can save this record, but check the create permission to see if we can return to make a new one.
                if ($user->authorise('core.create', 'com_marathonmanager'))
                {
                    $toolbarButtons[] = ['save2new', 'arrivaloption.save2new'];
                    $toolbarButtons[] = ['save2copy', 'arrivaloption.save2copy'];
                }
            }
        }

        ToolbarHelper::saveGroup($toolbarButtons);
		ToolbarHelper::cancel('arrivaloption.cancel', 'JTOOLBAR_CLOSE');
	}
}
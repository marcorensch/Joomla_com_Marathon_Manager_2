...
(
    ...
    `shortcode`        varchar(1)            DEFAULT NULL,
    `state`            tinyint(3)   NOT NULL DEFAULT 0,
    `published`        tinyint(1)   NOT NULL DEFAULT '1',
    `publish_up`       DATETIME,
    `publish_down`     DATETIME,
...
(
    ...
    CONSTRAINT `fk_com_marathonmanager_results_reg_id` FOREIGN KEY (`registration_id`) REFERENCES `#__com_marathonmanager_registrations` (`id`) ON DELETE SET NULL ON UPDATE CASCADE
);
